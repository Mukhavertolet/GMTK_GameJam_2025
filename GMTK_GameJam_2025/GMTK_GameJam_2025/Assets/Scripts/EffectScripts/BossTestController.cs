...
public class BossTestController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (timeSinceLastMove > randomMovementTime && !isMoving && !isFollowingPlayer)
        {
            isMoving = true;
            StartCoroutine(Move());
        ...
        if (isFollowingPlayer)
            transform.position = Vector2.MoveTowards(transform.position, target.transform.position, entityStats.moveSpd * Time.deltaTime);
